...
vec3 pRoll(inout vec3 p) {
    pR(p.xz, iGlobalTime * 0.1);
    vec2 mp = vec2(0.750965250965251, 0.825);
...
float fHole(
    ...
    float w = 1.;
    float h = 1.;
    float round = .08; // connection points of holes
    float thick = gapValue; // gap-value slider
...
float shell(vec3 p, float i) {
    ...
    float thick = thicknessValue; // value-thickness slider
    float round = .015;
...
const float MAX_TRACE_DISTANCE = 6.; // max trace distance (reduced for performance)
const float INTERSECTION_PRECISION = .005; // precision of the intersection (relaxed for performance)
const int NUM_OF_TRACE_STEPS = 50; // DRASTICALLY reduced for performance
const float FUDGE_FACTOR = .8; // Increased to take bigger steps (faster but less accurate)
...
void doCamera(out vec3 camPos, out vec3 camTar, out float camRoll, in float time, in vec2 mouse) {
    ...
    float zoom = zoomValue; // value-zoom slider
    camPos = zoom * orient;
    camTar = vec3(0);
}
